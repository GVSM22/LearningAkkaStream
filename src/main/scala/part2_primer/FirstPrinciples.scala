...
object FirstPrinciples extends App {
  ...
  implicit val system = ActorSystem("FirstPrincipal")
...
//  illegalSource.to(Sink.foreach(println)).run()
  ...
  val aFutureSource = Source.fromFuture(Future(42))
  ...
  val takeFlow = Flow[Int].take(5) // turns the stream to a final stream, takes the first 5 and end the stream
  // drop, filter
  // NOT have flatMap
...
//  mapSource.runForeach(println)
   ...
   * with length > 5 characters and then just print them out
   */

  Source[String](List("Giacomo", "Oi", "Alo", "Carlito", "Bob Esponja"))
    .filter(_.length > 5)
    .take(2)
    .runForeach(println)
